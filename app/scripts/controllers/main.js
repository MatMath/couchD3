...
angular.module('couchD3App').controller('MainCtrl', function($scope, $window) {
	...
	$scope.UpdateTheData = function(responseResult, UpdatedResult) {
		$scope.UpdatedResult = [{
				'key': responseResult.rows[0].key.BarName,
				'bar': true,
				'values': []
			}, {
				'key': responseResult.rows[0].key.LineName,
				'values': []
			}];

		$scope.UpdatedResult[0].values = responseResult.rows.map(function(row){
			return{
				x: row.key.timestamp,
				y: row.value.BarValue
			};
		});
		$scope.UpdatedResult[1].values = responseResult.rows.map(function(row){
			return{
				x: row.key.timestamp,
				y: row.value.LineValue
			};
		});
		// They use different Scope, How to fire them? 
	$scope.discreteChartData = $scope.UpdatedResult;
	};
});